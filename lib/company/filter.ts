...
export const COMPANY_FILTER_KEY = 'filter'
...
const companyFilter = derived(page, $page => {
	const optionalFilter = $page.query.get(
		COMPANY_FILTER_KEY
	) as CompanyFilter | null

	const filter = optionalFilter ?? DEFAULT_COMPANY_FILTER
	const valid = companyFilters.includes(filter)
	...
	return valid ? filter : DEFAULT_COMPANY_FILTER
})
